...
Page({
      ...
      async getList() {
        // console.log("getList");
            let that = this;
            try{
              const res = await request("/idle/all",{},"GET");
              console.log(res)
              wx.hideLoading();
              ...
              for (let i = 0; i < res.data.length; i++) {
                ...
                res.data[i].pictureList = pictureList.length > 0 ? pictureList[0] : '';
                res.data[i].releaseTime = res.data[i].releaseTime.substring(0,10);
                if (res.data[i].idleStatus == 1) {
                    app.dataList[0].push(res.data[i]);
                } else if (res.data[i].idleStatus == 2) {
                    app.dataList[1].push(res.data[i]);
                }
            }
              that.setData({
                nomore: true,
            ...
            wx.showModal({
                  ...
                  async  success(res) {
                        if (res.confirm) {
                              ...
                              try{
                                  const res = await request('/idle/update',{id:del.id,idleStatus:0},'POST')
                                  wx.hideLoading();
                                          wx.showToast({
                                                title: '删除成功',
                                          })
      ...
      detail(e) {
            ...
            wx.navigateTo({
              url: '/pages/detail/detail?scene=' + detail.id,
            })      
